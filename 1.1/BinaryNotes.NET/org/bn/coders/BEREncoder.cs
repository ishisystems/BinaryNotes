...
namespace org.bn.coders
{
	...
	{		
		...
		{
			int resultSize = 0, sizeOfString = 0;
            byte[] buffer = System.Text.UTF8Encoding.UTF8.GetBytes((string) obj);
            stream.Write(buffer, 0, buffer.Length);
            sizeOfString = buffer.Length;
			
			resultSize += sizeOfString;
			resultSize += encodeLength(sizeOfString, stream);
			resultSize += encodeTag(BERCoderUtils.getTagValueForElement(elementInfo, TagClasses.Universal, ElementType.Primitive, BERCoderUtils.getStringTagForElement(elementInfo)), stream);
			return resultSize;
		}
