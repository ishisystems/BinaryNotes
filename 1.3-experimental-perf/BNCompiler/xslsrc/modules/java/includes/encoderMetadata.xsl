    ...
    <xsl:template name="encoderMetadata">
        <xsl:choose>
            ...
            <xsl:when test="typeReference/isSequence = 'true'">
              ValueEncoders.SEQUENCE
            </xsl:when>
            <xsl:when test="typeReference/isChoice = 'true'">
              ValueEncoders.CHOICE
            </xsl:when>
