...
>
  ...
  <xsl:template name="choiceElementFunctions">
        ...
        public boolean is<xsl:call-template name="toUpperFirstLetter"><xsl:with-param name="input" select="$elementName"/></xsl:call-template>Selected () {
            return this.<xsl:value-of select="$elementName"/> != null;
        }

        private void set<xsl:call-template name="toUpperFirstLetter"><xsl:with-param name="input" select="$elementName"/></xsl:call-template> (<xsl:call-template name="elementType"/> value) {
